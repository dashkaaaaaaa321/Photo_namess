...
class Main(tk.Fame):
    ...
    def init_main(self):
        toolbal = tk.Fame(bg='#d7d8e0', bd=2)
        toolbal.pack(side=tk.TOP, fill=tk.X)
        self.add_img = tk.PhotoImage(file='./img/add.png')
        btn_open_dialog = tk.Button(toolbal, bg='#d7d8e0', bd=0,
                                    image=self.add_img, command=self.open_dialog)
        btn_open_dialog.pack(side=tk.LEFT)

        self.update_img = tk.PhotoImage(file='./img/update.png')
        btn_edit_dialog = tk.Button(toolbal, bg='#d7d8e0', bd=0,
                                    image=self.update_img, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(file='./img/delete.png')
        btn_delete = tk.Button(toolbal, bg='#d7d8e0', bd=0,
                               image=self.delete_img, command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)

        self.search_img = tk.PhotoImage(file='./img/search.png')
        btn_search = tk.Button(toolbal, bd='#d7d8e0', bd=0,
                               image=self.search_img, command=self.open_search_dialog)
        btn_search.pack(side=tk.LEFT)

        self.refresh_img = tk.PhotoImage(file='./img/refresh.png')
        btn_refresh = tk.Button(toolbal, bg="#d7d8e0", bd=0,
    ...
    def update_record(self, name, tel, email):
        self.db.c.execue('''UPDATE db SET name?, tel?, email? WHERE=ID?''',
    ...
    def delete_records(self):
        for selection_item in self.tree.selection():
            self.db.c.execute('''DELETE FROW db WHERE id=?''',
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_select = tk.Label(self, text='ФИО:')
        label_select.place(x=50, y=50)
        label_sum = tk.Label(self, text='E-mail')
        label_sum.place(x=50, y=110)

        self.enrty_name = ttk.Entry(self)
        self.enrty_name.place(x=200, y=50)
        
        self.enrty_email = ttk.Entry(self)
        self.enrty_email.place(x=200, y=80)

        self.enrty_tel = ttk.Entry(self)
        self.enrty_tel.place(x=200, y=110)

        self.btn_cancel = ttk.Button(
            self, text='Закрыть', command=self.destroy)
        self.btn_cancel.place(x=300, y=120)

        self.btn_ok = ttk.Button(self, text='Добавить')
        self.btn_ok.bind('<Button-1>', lambda event: self.view.records(self.enrty_name.get(),
                                                                       self.enrty_email.get(),
                                                                       self.enrty_tel.get()))
...
class Update(Child):
    ...
    def init_edit(self):
        ...
        btn_edit.bind('<Button-1>', lambda event: self.view.update_record(self.enrty_name.get(),
                                                                          self.enrty_email.get(),
                                                                          self.enrty_tel.get()))
    ...
    def default_data(self):
        ...
        self.enrty_name.insert(0, row [1])
        self.enrty_email.insert(0, row[2])
        self.enrty_tel.insert(0, row[3])
